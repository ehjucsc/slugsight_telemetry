...
# --- Path Configuration ---
# Calculate absolute path to gds/ directory for reliable logging
            ...
            with serial.Serial(port, 115200, timeout=1) as ser:
                ...
                while True: # Read loop
                    try:
                        ...
                        # 2. Log only if it is active flight data (has packet count)
                        if parsed_telemetry.get('packet_count', 0) > 0:
                            datalogger.write(parsed_telemetry)

                        # 3. Update global_data for the GUI
                        payload = None
                        with data_lock:
                            ...
                            # Update System Status (Active vs Waiting)
...
if __name__ == "__main__":
    ...
    # Initialize DataLogger with the absolute path we calculated earlier
    ...
    print(f"Open this URL in your browser: http://127.0.0.1:8080")
    print("-----------------------------------------------------")
    try:

"""
Telemetry Parser (v2.1)
Parses CSV telemetry data OR "Waiting" status messages.
"""
...
class TelemetryParser:
    ...
    def parse(self, raw_data: str) -> Optional[Dict[str, Any]]:
        ...
        # --- 1. Handle "Waiting for GPS" Message ---
        # The RX sends: "Waiting for GPS Fix...,-85"
        if "Waiting for GPS Fix" in raw_data:
            # Create a zeroed-out telemetry dict
            telemetry = {k: 0.0 for k in self.key_map.values()}
            telemetry['timestamp'] = datetime.now().isoformat()
            
            # Set a special flag for the GCS to read
            telemetry['sys_status'] = 'waiting' 
            
            # Extract RSSI if present (it's after the comma)
            parts = raw_data.split(',')
            if len(parts) > 1:
                try:
                    # Map the RSSI value to the correct key
        ...
        # --- 2. Handle Standard CSV Data ---
        try:
            values = raw_data.split(',')
            expected_len = len(self.DATA_LABELS)
            
            # Allow 17 (No RSSI) or 18 (With RSSI)
            ...
            telemetry = {
                ...
                'sys_status': 'active' # Normal operation
            }
